...
public class TSRMServer /*extends GridServiceImpl implements ISRMPortType*/{
	...
	if (Config._staticTokenList != null) {
	  for (int i=0; i<Config._staticTokenList.length; i++) {
	    StaticToken curr = ConfigXrootd._staticTokenList[i];
	...
	*/
	...
	if (SURLList == null) {
	    result.setReturnStatus(TSRMUtil.createReturnStatus(TStatusCode.SRM_SUCCESS, null));
    	} else if (fileStatusArray.length == SURLList.length) {
	    result.setReturnStatus(TSRMUtil.createReturnStatus(TStatusCode.SRM_SUCCESS, null));
    	} else {
	    result.setReturnStatus(TSRMUtil.createReturnStatus(TStatusCode.SRM_DONE, "Not all SURLs are found in the specified request."));
    	}
	*/
    	return result;
    }
	...
	*/
	...
	/*
    	if (fromSURLList == null) {
	    result.setReturnStatus(TSRMUtil.createReturnStatus(TStatusCode.SRM_SUCCESS, null));
    	} else if (fileStatusArray.length == fromSURLList.length) {
	    result.setReturnStatus(TSRMUtil.createReturnStatus(TStatusCode.SRM_SUCCESS, null));
	} else {
	    result.setReturnStatus(TSRMUtil.createReturnStatus(TStatusCode.SRM_DONE, "Not all SURLs are found in the specified request."));
    	}
