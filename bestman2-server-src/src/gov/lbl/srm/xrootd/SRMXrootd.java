...
public class SRMXrootd extends TSRMServerIdle {    
	...
	if (_compileDate == null) {
	    _compileDate = TSRMUtil.getCompiledDate(gov.lbl.srm.server.SRMXrootd.class);
	...
	for (int i=0; i<listOfFileReqs.length; i++) {	    
		...
		if (txfURL == null) {
			...
			if (size >= 0) {
			    ...
			    s.setStatus(TSRMUtil.createReturnStatusInvalidPath("Cannt find size info on the given path="+src+" via:"+txfURL.getPath()+" configuration option: "+ConfigXrootd._DefConfigCheckXrootdFS+"="+ConfigXrootd._doCheckSizeWithXrootdFS+" "+ConfigXrootd._DefConfigCheckSizeGsiftp+"="+ConfigXrootd._doCheckSizeWithGsiftp));
			}
		    }
		}
	...
	try {
	    ...
	    if (ConfigXrootd._doCheckSizeWithXrootdFS) {
		ff.setFileAccessFS();
...
/*
	...
	if (xrootdPath == null) {
	    ...
	    if (ConfigXrootd._doCheckSizeWithGsiftp) {
...
*/
	...
	if (ConfigXrootd._xrootdTokenCompName != null) {
	...
	TSRMLog.debug(SRMXrootd.class, null, "authid="+req.getAuthorizationID(), "fasttrack="+fastTrack);
	...
	if (op == SRMXrootd.DefDELETEFILE) {
	    sudoCommand +=" /bin/rm ";
	} else if (op == SRMXrootd.DefDELETEDIR) {
	    sudoCommand +=" /bin/rmdir  ";
	} else if (op == SRMXrootd.DefMKDIR) {
	...
	if (isAllLevelRecursive != null) {
	    if ((isAllLevelRecursive.booleanValue() == true) && (!ConfigXrootd._enableRecursionInLs)){
	...
	*/
	...
	if ((ConfigXrootd._doCheckSizeWithGsiftp) && (creds == null)) {
	...
	if (!TSRMUtil.isRefToSRM(getHostPort(input), ConfigXrootd._wsdlEndPoint, input)) {
	    srmInfo("Unrecognizable url:"+input+" for this server:"+ConfigXrootd._wsdlEndPoint);
	    return null;
	}
...
*/
	...
	try {
	    ...
	    URI result = new URI(TSRMTxfProtocol.getDefaultTransferProtocol().generateURI(path+"?"+ConfigXrootd._xrootdTokenCompName+"="+token));
	    return result;
	} catch (Exception e) {
	    e.printStackTrace();
	    return null;
	...
	if (port == -1) {
	    port = ConfigXrootd._securePort; // default;
	}
	String result = null;
	...
	result = ConfigXrootd._DefContactProtocolStr+"://"+host+":"+port+path;
