...
public SRMClientPing(String[] args, PrintIntf pIntf) {
  ...
  for(int i = 0; i < args.length; i++) {
    ...
    */
    ...
    else if(args[i].equalsIgnoreCase("-renew")) {
      isRenew = true;
    }
  ...
  */
  ...
  if(!logPath.equals("")) {
    ...
    if(idx != -1) {
      ...
      if(f.isDirectory()) {
        System.out.println("Given logpath is a directory, " + 
          "please give the full path with desired filename for -log");
        showUsage(false);
      }
      }catch(Exception e) {
	     util.printMessage("\nSRM-CLIENT:Exception from client="+e.getMessage(),logger,silent);
     ...
     logPath ="./srm-client-detailed.log";
      ...
      try {
         Class c = cl.loadClass("gov.lbl.adapt.srm.client.main.SRMClientPing");
 ...
 try {
  ...
  if(_debug) {
    ...
    util.printEventLog(_theLogger,"SRMPing-Log4jLocation",inputVec,silent,useLog);
  }
    ...
    if(xxtemp != null) {
     ...
     if(proxyFile.equals("")) {
       if(userCert.equals("") && userKey.equals("")) {
        ...
        e.printStackTrace();
         ...
         if(userCert.equals("") || userKey.equals("")) {
           util.printMessage
		     ("\nUserCert and UserKey both should be provided",logger,silent);
           util.printMessage
		     ("\nUserCert and UserKey both should be provided",pIntf);
           inputVec.clear(); 
		   inputVec.addElement
     ...
     if(isRenew) {
      ...
      if(v1.startsWith("Enter") || v2.startsWith("Enter")) {
        util.printMessage("\nIf you want to renew proxy automatically,\n "+
         "you need to enter user-cert location and user-key location.",logger,silent);
        util.printMessage("\nIf you want to renew proxy automatically,\n "+
         "you need to enter user-cert location and user-key location.",pIntf);
        inputVec.clear(); 
      ...
      if(v3.startsWith("Enter")) {
        util.printMessage("\nIf you want to renew proxy automatically,\n "+
          "please enter your proxy file location.",logger,silent);
        util.printMessage("\nIf you want to renew proxy automatically,\n "+
          "please enter your proxy file location.",pIntf);
        inputVec.clear(); 
        inputVec.addElement("If you want to renew proxy automatically, "+
     ...
     try {
       ...
       proxyType=gov.lbl.adapt.srm.client.util.Util.getProxyType(properties);
        ...
        ee.printStackTrace();
        util.printMessage("SRM-PING:Exception="+ee.getMessage(),pIntf);
        if(pIntf == null) {
		inputVec.clear(); 
     ...
     util.printEventLog(_theLogger,"SrmPing",inputVec,silent,useLog);
     ...
     util.printMessage("Using proxyFile : " + proxyFile,logger,silent);

     util.printMessageHL("SRM Configuration", pIntf);
     util.printMessage("\nserviceUrl     : " + serviceUrl,pIntf);
     util.printMessage("Debug ON       : " + _debug,pIntf);
     util.printMessage("Quiet ON       : " + silent,pIntf);
     util.printMessage("Using proxyFile : " + proxyFile,pIntf);


	 inputVec.clear(); 
     inputVec.addElement("serviceurl="+serviceUrl);
     inputVec.addElement("DebugOn="+_debug);
     inputVec.addElement("QuietOn="+silent);
     ...
     if(pIntf == null) {
	  ...
	  util.printMessage("\nSRM-CLIENT:Exception from client="+e.getMessage(),logger,silent);
      util.printMessage("SRM-PING:Exception="+e.getMessage(),pIntf);
                    util.printEventLogException(_theLogger,"",e);
      e.printStackTrace();
      int idx = e.toString().indexOf("Unauthorized");
      if(idx != -1) {
       if(pIntf == null) {
	     inputVec.clear(); 
         inputVec.addElement("StatusCode=52");
         util.printEventLog(_theLogger,"StatusExit",inputVec,silent,useLog);
         System.exit(52);
       }
       else {  
