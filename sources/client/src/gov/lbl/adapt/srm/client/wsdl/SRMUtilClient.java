...
import gov.lbl.adapt.srm.client.intf.PrintIntf;
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    ...
    public SRMUtilClient (
			  String serverUrl, String uid, String userDesc, 
...
// doSrmMkdir
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
public boolean localMkdir(Vector fileInfo, StringBuffer sCode) {
      /*_srmClientIntf*/SRMClientN.logMsg("Calling local mkdir ", null, "\nSRM-DIR: "+new Date());
      /*_srmClientIntf*/SRMClientN.logMsg("  Total dir to create:"+fileInfo.size(), null, "\nSRM-DIR: ");
	  ...
	  try {  
	      ...
	      if(f.exists()) {
		  /*_srmClientIntf*/SRMClientN.logMsg(" file or directory already exists "+surl, null, "\nSRM-DIR:");
		  count++;
		  //sCode.append("SRM_FAILURE");  	      
	      } else {
		  boolean b = f.mkdirs();
		  if(b) {
		      /*_srmClientIntf*/SRMClientN.logMsg(" directory created "+surl, null, "\nSRM-DIR:");
		      count ++;
		      //sCode.append("SRM_SUCCESS");		  
		  } else {
		      /*_srmClientIntf*/SRMClientN.logMsg(" directory not created "+surl, null, "\nSRM-DIR:");
		      //sCode.append("SRM_FAILURE");
		  }
	      }
	  } catch(Exception e) {
              util.printEventLogException(_theLogger,"",e);
	      /*_srmClientIntf*/SRMClientN.logMsg(" Exception="+e.getMessage(), null, "\nSRM-DIR:");
	  }
      }
    ...
    public boolean gsiftpMkdir(Vector fileInfo, StringBuffer sCode, boolean skipTopDirectoryExistenceForRecursiveOption) 
    {
	/*_srmClientIntf*/SRMClientN.logMsg("Calling gsiftp mkdir ", null, "\nSRM-DIR:"+new Date());
	/*_srmClientIntf*/SRMClientN.logMsg("Total dire to create ", null, "\nSRM-DIR:"+new Date());
	...
	for(int i = 0; i < fileInfo.size(); i++) {
		...
		System.out.println("surl="+surl);
		MyGridFTPClient gc = new MyGridFTPClient(surl,_credential, logger,_theLogger,false);
		
		boolean b = gc.doMkdir(path);
		if(b) {
		    count++;
		    /*_srmClientIntf*/SRMClientN.logMsg("Directory created "+surl+"/"+path, null, "\nSRM-DIR:");
		} else {
		    if(skipTopDirectoryExistenceForRecursiveOption) {
			/*_srmClientIntf*/SRMClientN.logMsg("Please ignore the warnings", null, "\nSRM-DIR:");
			count++;		    
		    } else {
			/*_srmClientIntf*/SRMClientN.logMsg("Directory not created "+surl+"/"+path, null, "\nSRM-DIR:");
		    }
		}
	    } catch(Exception e) {
		util.printEventLogException(_theLogger,"",e);
...
private boolean srmMkdir(Vector fileInfo, StringBuffer sCode) throws Exception 
{
    String tag = "callSrmMkdir";
    	ExtendedGSSCredential cred = (ExtendedGSSCredential) _credential;
	byte[] bb = cred.export(ExtendedGSSCredential.IMPEXP_OPAQUE);
	String proxyString = new String(bb);
	
	/*_srmClientIntf*/SRMClientN.logMsg("Calling SrmMkdir ", null, "\n SRM-DIR:");
	
	if(_debug) {
	    /*_srmClientIntf*/SRMClientN.logMsg("Input Parameters for SrmMkdir .. ", null, "SRM-DIR:");
	    /*_srmClientIntf*/SRMClientN.logMsg("AuthorizationID="+uid, null, "SRM-DIR:");
	}
	...
	for(int i = 0; i < fileInfo.size(); i++) {
	    ...
	    /*_srmClientIntf*/SRMClientN.logMsg("DirectoryPath("+i+")="+turl, tag, "SRM-DIR:");
	    
	    boolean ok = mkdir (path, proxyString,storageInfo,sCode);
...
public boolean doSrmMkdir(Vector fileInfo,StringBuffer sCode, boolean doLocalLsList, boolean doGsiFTPList,boolean mkdiroption,			  
			  boolean skipTopDirectoryExistenceForRecursiveOption) 
    throws Exception 
{    
    boolean ok=false;
    if(doLocalLsList) {
	return localMkdir(fileInfo, sCode);    
    } else if(doGsiFTPList) {
	return gsiftpMkdir(fileInfo, sCode, skipTopDirectoryExistenceForRecursiveOption);
    } else if(mkdiroption) {
	return srmMkdir(fileInfo, sCode);
    } else {  	
	String tag = "callSrmMkdirOnSrc"; // I donnt think this is ever reached
	ExtendedGSSCredential cred = (ExtendedGSSCredential) _credential;
	byte[] bb = cred.export(ExtendedGSSCredential.IMPEXP_OPAQUE);
	String proxyString = new String(bb);
	...
	for(int i = 0; i < fileInfo.size(); i++) {
	    ...
	    /*_srmClientIntf*/SRMClientN.logMsg("DirectoryPath("+i+")="+surl, tag, "SRM-DIR:");
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    ...
    private TStatusCode doGsiFTPList(boolean recursive, boolean copyCase, Vector resultVec, String outputFile, Vector fileInfo, String targetDir, String sourceUrl) throws Exception {
    TStatusCode sCode = null;
		...
		try {
		    RemoteHostInfo remoteHostInfo = new RemoteHostInfo(tsurl,_credential,logger,_theLogger,			
...
private void doLocalCp(String targetDir, String sourceUrl) throws Exception {
			...
			idx = sourceUrl.lastIndexOf("/");
			  ...
			  String gtemp = SRMUtilClient.parseLocalSourceFileForPath (tempTargetDir);			      
			  try {
			      File f = new File(gtemp+"/"+ppath);
			      if(!f.exists()) {
				  f.mkdir();
				  System.out.println("SRM-CLIENT:Destination parent directory created " + gtemp+"/"+ppath);
			      } else {
				  if(f.isDirectory()) {
				      System.out.println("SRM-CLIENT:Destination parent directory already exists" 
							 + gtemp+"/"+ppath);                   
				  } else {
				      System.out.println("SRM-CLIENT:Destination path exists and it is a file" + gtemp+"/"+ppath);
				      System.exit(92);
				  }
			      }
			  }catch(Exception e) {               
			      e.printStackTrace();
			      util.printEventLogException(_theLogger,"",e);
			      System.out.println("SRM-CLIENT:Destination parent directory not created " + gtemp+"/"+ppath);
			  }
		      }
		  }	      
...
private void createMatchingSubdirRemotely(String targetDir, String sourceUrl, boolean copyCase) throws Exception {
    int idx = targetDir.lastIndexOf("/");
    if(idx != -1) {
	String tempTargetDir = targetDir.substring(0,idx);
	RemoteHostInfo rrInfo = new RemoteHostInfo (tempTargetDir, _credential, 	
						    logger, _theLogger, false,true,copyCase,false); 
	if (sourceUrl.endsWith("/")) {
	    idx = sourceUrl.lastIndexOf("/", sourceUrl.length()-2);
	} else {
	    idx = sourceUrl.lastIndexOf("/");
		      }
	if(idx != -1) {
	    ...
	    String msg = "SRM-CLIENT: Destination parent directory";
	    if(val == 0) {
		System.out.println(msg+" created " + tempTargetDir+"/"+ppath); 
	    } else {
		if(val == 1) {   
		    System.out.println(msg+" could not be created " + tempTargetDir+"/"+ppath);
		} else if (val == 2) {
		    System.out.println(msg +" already exists " + tempTargetDir+"/"+ppath);
		}
	    }
	}
    }
    ...
    private SrmLsRequest createLsRequest(TFileStorageType fileType, String tag, int numLevels, int count ,int offset,
					 boolean recursive, String proxyString, boolean fullDetailedList, Vector fileInfo) 
	throws Exception
    {
	    /*_srmClientIntf*/SRMClientN.logMsg(" Calling srmLsRequest", null, "\nSRM-DIR: "+new Date());

	    if(_debug) {
		/*_srmClientIntf*/SRMClientN.logMsg("\nSRM-DIR: ..........................", null, "");
		/*_srmClientIntf*/SRMClientN.logMsg("SRM-DIR: .....Input parameters for SrmLsRequest ....", null, "");
	    }
	    
	    String[] msgs = {"AuthorizationID="+uid, "FullDetailedList="+fullDetailedList,
			     "Recursive="+recursive, "NumOfLevels="+numLevels, "Offset="+offset, "Count="+count};
	    if (_debug) {
		/*_srmClientIntf*/SRMClientN.logMsg(msgs, tag, "SRM-DIR:");
	    }
		...
		/*_srmClientIntf*/SRMClientN.logMsg(" SURL="+ uri.toString(), tag, "SRM-DIR:");			      			     
		path[i] = uri;
	    }
	  
	    /*_srmClientIntf*/SRMClientN.logMsg("UseStorageSystemInfo="+storageInfo, tag, "SRM-DIR:");
		...
		while(stk.hasMoreTokens()) {
			...
			String value = temp.substring(idx+1);
			tExtra = new TExtraInfo();
			if(key.equals("source") || key.equals("for") ||	key.equals("login") || key.equals("passwd") ||			 
			   key.equals("projectid") || key.equals("readpasswd") ||  key.equals("writepasswd")) 
			{
			    tExtra.setKey(key);
			    tExtra.setValue(value);
			    vec.addElement(tExtra);		      
			} else {
			    throw new Exception("Given storage info is not in the correct format " + storageInfo);
			}		  
		    } else {
			throw new Exception("Given storage info is not in the correct format " + storageInfo);
		    }
		...
		/*_srmClientIntf*/SRMClientN.logMsg("FileStorageType="+fileType, tag, "SRM-DIR:");
	    }	    
	    return req;
}
			   ...
			   String targetDir,String sourceUrl) throws Exception 
{
	...
	if(doGsiFTPList) {
	    return doGsiFTPList(recursive, copyCase, resultVec, outputFile, fileInfo, targetDir, sourceUrl);
	} else if(doLocalLsList) {
		...
		if(targetDir.startsWith("file:") && sourceUrl.startsWith("file:")) { 
		    doLocalCp(targetDir, sourceUrl);
	      } else {
		    createMatchingSubdirRemotely(targetDir, sourceUrl, copyCase);
	      }//end else 
	    }//end if(copyCase)
		...
		try {
		    LocalListInfo localListInfo = new LocalListInfo(tsurl,logger,_theLogger,recursive,false);
	    ...
	    SrmLsRequest req = createLsRequest(fileType, tag, numLevels, count, offset, recursive, proxyString, fullDetailedList, fileInfo);
	    SrmLsResponse result = null;	    
	    result = (SrmLsResponse) callSoapThread(req,result,"srmls");

	    /*_srmClientIntf*/SRMClientN.logMsg("....................", null, "\nSRM-DIR:");
	    /*_srmClientIntf*/SRMClientN.logMsg("#### Output from SRM ####", null, "\nSRM-DIR:");
	    
	    if(result == null) {
		/*_srmClientIntf*/SRMClientN.logMsg("\nNull result from srmLs", tag, "");
		if(pIntf != null) {
		    pIntf.setCompleted(false);
		}
		return sCode;
	    ...
	    if(result.getReturnStatus () == null) {
		/*_srmClientIntf*/SRMClientN.logMsg("\nNull ReturnStatus from srmLs", tag, "");
		if(pIntf != null) {
		    pIntf.setCompleted(false);
		}
		return sCode;
	    ...
	    /*_srmClientIntf*/SRMClientN.logMsg("\tStatus =" + sCode+" Explanation=" + result.getReturnStatus().getExplanation(), tag, "");
	  
	    HashMap resultMap = new HashMap();
	    resultMap.put(uid, sCode);
	    String rToken = result.getRequestToken();

	    /*_srmClientIntf*/SRMClientN.logMsg("\tRequest token="+rToken, tag, "");
	    
	    TMetaDataPathDetail[] pathDetail = null;
	    
	    if(!submitOnly) {
		if(sCode == TStatusCode.SRM_REQUEST_QUEUED || sCode == TStatusCode.SRM_REQUEST_INPROGRESS) 
		{		   
		    int i = 0;
		    long sTimeStamp = util.startTimeStamp();
			...
			/*_srmClientIntf*/SRMClientN.logMsg("NextStatus="+tStatusTime+" seconds.", tag, "\nSRM-DIR:");
			...
			if(rToken != null) {
				...
				/*_srmClientIntf*/SRMClientN.logMsg("Max retry check status exceeded for ls status", tag, "SRM-DIR:");
				timedOutCase=true;
			    }
			    
			    if(!timedOutCase) {
				SrmStatusOfLsRequestRequest lsStatusRequest = new SrmStatusOfLsRequestRequest();
				...
				/*_srmClientIntf*/SRMClientN.logMsg(" Calling SrmStatusOfLsRequest " , null, "SRM-DIR: "+new Date());
				...
				/*_srmClientIntf*/SRMClientN.logMsg("SrmStatusOfLsResponse", tag, null);				
				/*_srmClientIntf*/SRMClientN.logMsg("SRM-DIR: .......................", null, "");

				if(lsStatusResponse != null) {  
				    if(lsStatusResponse.getReturnStatus() != null) {
					rStatus = lsStatusResponse.getReturnStatus();
					sCode = rStatus.getStatusCode();					
					/*_srmClientIntf*/SRMClientN.logMsg("Status="+sCode.toString()+" Explanation="+rStatus.getExplanation(), tag, "SRM-DIR");
					if(sCode != TStatusCode.SRM_REQUEST_INPROGRESS && sCode != TStatusCode.SRM_REQUEST_QUEUED) {
					    pathDetail = lsStatusResponse.getDetails().getPathDetailArray(); 
					    resultMap.remove(uid); }
				    } else {
					/*_srmClientIntf*/SRMClientN.logMsg("Null return status from srmStatusOfLsRequest", tag, "\nSRM-DIR:");
				    }				
				} else {
				    /*_srmClientIntf*/SRMClientN.logMsg("Null response from srmStatusOfLsRequest", tag, "\nSRM-DIR:");
				}			    
			    } else {
				resultMap.remove(uid);
			    }			
			} else {
			    /*_srmClientIntf*/SRMClientN.logMsg("Expecting requestToken for this status code for lsRequest from SRM", tag, "\nSRM-DIR:");
			    resultMap.remove(uid);
			}
		...
		if(result.getDetails() != null) {
		    ...
		    /*_srmClientIntf*/SRMClientN.logMsg("\nSRM-DIR: SRM returned null getDetails for srmLs", null, "");
		    if(pIntf != null) {
			pIntf.setCompleted(false);
		    }
		    return sCode;
	    ...
	    if(pathDetail == null) {
		/*_srmClientIntf*/SRMClientN.logMsg("\nSRM-DIR: SRM returned null pathDetails for srmLs", null, "");
		if(pIntf != null) {
		    pIntf.setCompleted(false);
		}
		return sCode;
	    ...
	    if(textReport) {
		/*_srmClientIntf*/SRMClientN.logMsg("Printing text report now", tag, "\nSRM-DIR:");
		...
		/*_srmClientIntf*/SRMClientN.logMsg("SRM-CLIENT*REQUEST_STATUS="+rStatus.getStatusCode().toString(), null, "");
		if(rStatus.getExplanation() != null) {
		    /*_srmClientIntf*/SRMClientN.logMsg("SRM-CLIENT*REQUEST_EXPLANATION="+ rStatus.getExplanation(), null, "");
		}
		for(int i = 0; i < pathDetail.length; i++) {
		    printMetaDataForTextReport("\t",pathDetail[i],getRecursive, surl,turl);
		}
		/*_srmClientIntf*/SRMClientN.logMsg("PrintedTextReport", tag, null);
	...
	util.printEventLogException(_theLogger,"",e);
	/*_srmClientIntf*/SRMClientN.logMsg("Exception="+e.getMessage(), tag, "SRM-DIR:");
	...
	if(msg.startsWith("CGSI-gSOAP: Could not find mapping") || idx != -1 || idx1 != -1 || idx5 != -1) 
	{
	    util.printMessage("\nException : "+msg,logger,silent);
	    util.printMessageHException("\nException : "+msg,pIntf);
	    if(pIntf == null) {
		if(idx != -1) {
		    /*_srmClientIntf*/SRMClientN.logMsg("ExitStatus="+90, tag, null);
		    System.exit(90);		
		} else if(idx1 != -1 || idx5 != -1) {
		    util.printMessage("Exception : proxy type mismatch " +" please check your proxy", logger,silent);
		    /*_srmClientIntf*/SRMClientN.logMsg("ExitStatus="+96, tag, null);
		    System.exit(96);
		} else {
		    /*_srmClientIntf*/SRMClientN.logMsg("ExitStatus="+91, tag, null);
		    System.exit(91);
		}
	    }	
	} else {
   ...
   if(spaceStatusResponse != null) {
     if(spaceStatusResponse.getReturnStatus() != null) {
       ...
       if(spaceStatusResponse.getRetentionPolicyInfo() != null) {
        ...
        iVec.addElement("Null return status from srmStatusOfReserveSpaceRequest");
        util.printEventLog(_theLogger,"SrmStatusOfReserveSpaceResponse",iVec,silent,useLog);
        System.out.println("SRM-CLIENT: Null return status " +"from srmStatusOfReserveSpaceRequest");
     }
   } else {
        System.out.println("SRM-CLIENT: return null result from srmStatusOfReserveSpaceRequest");
   }
   return sCode;
}
