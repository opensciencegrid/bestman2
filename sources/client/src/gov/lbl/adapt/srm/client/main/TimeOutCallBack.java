...
public class TimeOutCallBack extends Thread {
	...
	SRMClientN.logMsg("init: sec="+connectionTimeOutAllowed, "TimeOut for:"+callingMethodName, null);
    }
    ...
    public void run () {  
	try {
	    String tag = "TimeOutCallBack";
	    SRMClientN.logMsg(callingMethodName+" Date="+new Date()+" TimedOut=start "+Thread.currentThread(), tag, null);
	    while(true) {
		sleep(1000); 
		if(obj != null) { 
		    interrupt();
		    ...
		    boolean b = util.isConnectionTimeOut(startTimeStamp,connectionTimeOutAllowed);		     
			...
			if(obj == null) {
			    SRMClientN.logMsg(callingMethodName+" Date="+new Date()+" TimedOut=true."+Thread.currentThread(), tag, null);
			}					    
			interrupt();
			return;
		    }
