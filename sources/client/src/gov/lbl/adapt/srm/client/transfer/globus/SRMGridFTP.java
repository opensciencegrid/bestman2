...
public void transferSync(String fromu, String tou) {
    ...
    try {
        ...
        if (tmode == SRMTransferMode.GET) {	    
	    gov.lbl.adapt.srm.client.main.SRMClientN.logMsg("GridFTP GET:", tag, null);
		...
		String msg = "GET: comparing size: " + tourl.getPath() + "  " + remoteSize + " == " + f.length();
		gov.lbl.adapt.srm.client.main.SRMClientN.logMsg(msg, tag, null);
            }        
        } else if (tmode == SRMTransferMode.PUT) {            
	    gov.lbl.adapt.srm.client.main.SRMClientN.logMsg("GridFTP PUT:", tag, null);
	...
	String msg = Thread.currentThread()+" DONE w/ Gridftp: " + fromurl.getPath() + " time took (milliseconds) =" + (ft-st);	
	gov.lbl.adapt.srm.client.main.SRMClientN.logMsg(msg, tag, null);
...
public void transferSync() {
    String tag = "GridFTP.transferSync";
    String msgStart[] = {"From=" + fromurl.getURL(), "To=" + tourl.getURL(), 
			 "client specified dcau="+userdcau, "local server mode="+localServerMode,
			 "parallelism="+parallelism, "buffersize="+buffersize, "blocksize="+blocksize};
    gov.lbl.adapt.srm.client.main.SRMClientN.logMsg(msgStart, tag, null);
	...
	if (tmode == SRMTransferMode.GET) {
	    ...
	    // differ, otherwise they shouldn't
	    if (transferType != Session.TYPE_ASCII) {
		localFile = new File(tourl.getPath());
		localSize = localFile.length();

		String msg = "GET: comparing size: " + tourl.getPath() + " " + remoteSize + " == " +localFile.length(); 
		gov.lbl.adapt.srm.client.main.SRMClientN.logMsg(msg, tag, null);
	    }	
	} else if (tmode == SRMTransferMode.PUT) {
	    gov.lbl.adapt.srm.client.main.SRMClientN.logMsg("GridFTP PUT", tag, null);
	    ...
	    // differ, otherwise they shouldn't
	    if (transferType != Session.TYPE_ASCII) {
		localFile = new File(fromurl.getPath());

		String msg = "PUT: comparing size: " + fromurl.getPath() + " " + remoteSize + " == " + localFile.length();
		gov.lbl.adapt.srm.client.main.SRMClientN.logMsg(msg, tag, null);
	    }	
	} else if (tmode == SRMTransferMode.THIRDPARTY) {
	    thirdparty();
	...
	String msg = Thread.currentThread()+" Done w/ Gridftp: " + fromurl.getPath() + " took (milliseconds) =" + (ft-st);	
	gov.lbl.adapt.srm.client.main.SRMClientN.logMsg(msg, tag, null);

	String ptmMsg = "PTM "+localFile.length()+" "+txfTimeInMilliSeconds+" "+parallelism;
	gov.lbl.adapt.srm.client.main.SRMClientN.logMsg(ptmMsg, tag, null);
	
	//gov.lbl.adapt.srm.client.main.SRMPassiveTransferMonitor.getInstance().update(localFile.length(), txfTimeInMilliSeconds, parallelism); 
	gov.lbl.adapt.srm.client.main.IPassiveTransferMonitor ptm = gov.lbl.adapt.srm.client.main.SRMClientN.getPTM();
	
	if (ptm != null) {
	    ptm.update(localFile.length(), txfTimeInMilliSeconds, parallelism); 
	} else {
	    System.out.println("\n\n no PTM \n\n");
	}
    } catch (Exception e) {
...
public void put() throws IOException {
	...
	// 451 gridftp error
	if (client.isFeatureSupported("DCAU")) {
	    gov.lbl.adapt.srm.client.main.SRMClientN.logMsg("### gsiftp dcau on", tag, null);
	    ...
	    gov.lbl.adapt.srm.client.main.SRMClientN.logMsg("### gsiftp dcau off", tag, null);
	    //client.setDataChannelAuthentication(DataChannelAuthentication.NONE);
	    client.setLocalNoDataChannelAuthentication();
	}
