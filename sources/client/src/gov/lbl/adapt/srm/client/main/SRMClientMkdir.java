...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
// Class SRMClientMkdir
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::

public class SRMClientMkdir
{
...
public SRMClientMkdir(String[] args, PrintIntf pIntf) {
  ...
  */
  ...
  if(!logPath.equals("")) {
    ...
    if(idx != -1) {
      ...
      try {
         Class c = cl.loadClass("gov.lbl.adapt.srm.client.main.SRMClientMkdir");
  ...
  */
  ...
  try {
    ...
    if(inputFile.equals("")) { 
        ...
        //surl[0] = sourceUrl;
        request = gov.lbl.adapt.srm.client.util.Util.createRequest(sourceUrlVec,"","",_debug,silent,useLog,
            "SRM-DIR",false,_theLogger,logger);
      request.setModeType("dir");
    } 
    else {
      //by default request is get, so target dir check is done in parseXML
      request = gov.lbl.adapt.srm.client.util.Util.parseXML(inputFile,"SRM-DIR",silent,useLog,_theLogger);
      request.setModeType("dir");
    }
    ...
    if(serviceUrl.equals("") && !doLocalLsList && !doGsiFTPList) {
      String temp = "";
      if(fileInfo.size() > 0) { 
    ...
    if(!doLocalLsList && !doGsiFTPList) {
      ...
      inputVec.addElement("doLocalLsList="+inputFile);
      inputVec.addElement("doGsiFTPList="+inputFile);
      ...
      */
      ...
      utilClient.doSrmMkdir(fileInfo,sCode,doLocalLsList,doGsiFTPList,false,false);
      if(pIntf == null) {
        inputVec.clear();
        inputVec.addElement("Status.str.trim=|"+sCode.toString().trim()+"|");
        util.printEventLog(_theLogger,"StatusCode",inputVec,silent,useLog);
      }
      int exitValue = util.mapStatusCode(sCode.toString()); 
      if(pIntf == null) {
        inputVec.clear();
        inputVec.addElement("Status=|"+sCode.toString().trim()+"|");
...
private Vector  validateURL(Request request) {
  ...
  try {
    ...
    util.printMessageHException("SRM-DIR: Exception="+e.getMessage(),pIntf);
    ...
    if(request.getModeType().equalsIgnoreCase("dir")) {
       if(!surl.startsWith("srm://") && !doLocalLsList && !doGsiFTPList) {
          ...
          util.printMessage("\nSRM-DIR: skipping this url in the request", 
				logger,silent);
          util.printMessage("\nSRM-DIR:skipping this url in the request",pIntf);
          skip = true;
       }
       else if(surl.startsWith("srm://")) {
          simpleURL=true;
       }
       else if(simpleURL && doLocalLsList || simpleURL && doGsiFTPList) {
          inputVec.clear();
          inputVec.addElement("Mixed source url is not allowed " + surl);
          inputVec.addElement(" skipping this url in the request");
          util.printEventLog(_theLogger,"ValidateURL",inputVec,silent,useLog);
...
public void showUsage (boolean b) {
 if(b) { 
    System.out.println("Usage :\n"+
            "\tor srm-mkdir <sourceurl> [command line options]\n" +
            "\tor srm-mkdir [command line options] -s <source_url>\n" +
            "\tor srm-mkdir [command line options] -s <source_url> -s <source_url> \n" +
            //"\tor srm-mkdir [command line options] -s <source_url>&<source_url> \n" +
            "\tor srm-mkdir [command line options] -f <file>\n" +
