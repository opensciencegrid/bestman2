...
public class MyGlobusURL {
  ...
  private String removeAnyExtraSlashInGsiftpURL(String surl) {
    ...
    private void getPort(String tt, String ss) throws Exception {	
	if(!tt.equals("")) {
	    try {
		port = Integer.parseInt(tt);
	    }catch(NumberFormatException nfe) {
		throw new Exception("Given port is not valid number " + tt+" in: "+ss);
	    }
	}
    }
  ...
  private void parseServiceHandle (String ss, int type) throws Exception {
      ...
      //if (idx1 != -1) {
      if (!protocol.equalsIgnoreCase("file") && (idx1 != -1)) {
