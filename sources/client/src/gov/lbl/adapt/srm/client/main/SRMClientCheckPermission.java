  ...
  try {
    ...
    String temp="";
       ...
       if(inputFile.equals("")) { 
          ...
          surl[0] = sourceUrl;
          request = 
				gov.lbl.adapt.srm.client.util.Util.createRequest(surl[0],"","",_debug,silent,useLog,
                 "SRM-UTIL",false,_theLogger,logger);
       } 
       else {
        //by default request is get, so target dir check is done in parseXML
         request = 
			gov.lbl.adapt.srm.client.util.Util.parseXML(inputFile,"SRM-UTIL",silent,useLog,_theLogger);
       }
