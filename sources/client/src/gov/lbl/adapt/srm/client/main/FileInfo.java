...
public class FileInfo implements FileIntf{
  ...
  public boolean getDoNotReleaseFile() {
    ...
    public void use(String url) throws Exception  { // added by junmin
	gov.lbl.adapt.srm.client.util.MyGlobusURL gurl = new gov.lbl.adapt.srm.client.util.MyGlobusURL(url,0);
	String path = gurl.getPath();
	String host = gurl.getHost();
	int port = gurl.getPort();
	setSURL("gsiftp://"+host+":"+port+path);
	if(!path.startsWith("//")) {
	    setSURL("gsiftp://"+host+":"+port+"/"+path);
	}
    }
