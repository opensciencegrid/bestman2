...
public static GSSCredential createProxy (XMLParseConfig pConfig, Properties properties,
 ...
 GSSCredential mycred = gov.lbl.adapt.srm.client.util.Util.getCredential(properties,
...
public static GSSCredential checkTimeLeft (XMLParseConfig pConfig,
  ...
  GSSCredential mycred = gov.lbl.adapt.srm.client.util.Util.getCredential(
  ...
  try {
    ...
    if(remainingLifeTime <= 1800) { //if <= 30 minutes
      if(gov.lbl.adapt.srm.client.util.Util.isRenewProxy(_password)) {
        try {
          mycred = gov.lbl.adapt.srm.client.util.Util.createProxy(pConfig,properties,_password,
...
public static GSSCredential getCredential(Properties properties, 
   ...
   try {
     if(!proxyPath.startsWith("Enter")) {
       ...
       mycred =
        gov.lbl.adapt.srm.client.util.Util.getCredential(proxyPath,"",useProxy,"");
     }
      else {
       ukey = properties.getProperty("user-key");
       ucert = properties.getProperty("user-cert");
       ...
       if(!ukey.startsWith("Enter") && (!ucert.startsWith("Enter"))) {
         ...
         mycred =
            gov.lbl.adapt.srm.client.util.Util.getCredential(ucert,ukey,
                useProxy,_password);
       }
       else {
          inputVec = new Vector ();
