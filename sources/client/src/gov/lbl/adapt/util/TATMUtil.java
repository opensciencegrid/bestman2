...
public class TATMUtil {
   ...
   public static final TATMMutex _tokenCounterMutex                 = new TATMMutex();
   ...
   public static String generatePathStr(String tokenID, String filename) {
       return tokenID+"-"+TATMUtil.getNextRandomUnsignedInt()+"/"+filename;
   }
   ...
   public static int getNextRandomUnsignedInt() {
    ...
    public static boolean acquireSync(TATMMutex m) {
    	try {
	    m.acquire();
	    return true;
    ...
    public static boolean releaseSync(TATMMutex m) {
    	m.release();
    	return true;
    }
	...
	try {
	    ...
	    //TATMLog.exception(TATMUtil.class, "parseTimeStr()"+timeStamp, e);
	    //e.printStackTrace();
	...
	if (TATMUtil.acquireSync(_tokenCounterMutex)) {
	    _numTokensIssued++;
	    TATMUtil.releaseSync(_tokenCounterMutex);
	...
	//TATMLog.info(TATMUtil.class, null, "event=sync", "_numTokens="+_numTokensIssued+" expected="+expectedCounter);
	...
	return f.getPath(); // full surl is: TATMTxfProtocol.FILE.generateURI(f).toString()
    }
  ...
  public static Calendar createGMTTime(String longisoDate, String longisoTime) {
	  ...
	  return createGMTTime(d.getTime());
	    ...
	    //TATMLog.exception(TATMUtil.class, "parsingLongIso:"+longisoDate+" "+longisoTime, e);
	...
	//TATMLog.debug(TATMUtil.class, null, "event=isRefToSRM surl=\""+surl+"\" srmURI="+srmURI.toString(), "incoming="+incomingUrl+" noServicePathCheckFlag="+noServicePathCheck);
	//TATMLog.debug(TATMUtil.class, null, "event=isRefToSRM surl=\""+surl+"\" srmURI="+srmURI.toString(), "incoming="+incomingUrl);
	try {
		...
		if (!srmServerIP.equalsIgnoreCase(inputIP)) {
		    //TATMLog.debug(TATMUtil.class, null, "event=isRefToSRM surl=\""+surl+"\" ip="+inputIP, "serverIP="+srmServerIP);
		    return false;
		}
	    }
	    if (input.getPort() != srmURI.getPort()) {
		//TATMLog.debug(TATMUtil.class, null, "event=isRefToSRM surl=\""+surl+"\" inputPort="+input.getPort(), "serverPort="+srmURI.getPort());
		return false;
	    }
	    ...
	    TATMLog.debug(TATMUtil.class, null, "event=isRefToSRM surl=\""+surl+"\" path="+input.getPath(), "serverPath="+srmURI.getPath());
	} catch (Exception e) {
	    e.printStackTrace();
	    return false;
	}
...
class MaskingThread extends Thread {
	...
	if (obj == null) {
	    //throw new TATMException("Unexpected error, "+msg, false);
	}
     }
