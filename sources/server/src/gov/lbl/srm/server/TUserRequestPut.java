...
public class TUserRequestPut extends TUserRequest {
    ...
    public void cleanUp() {	
	TStatusCode code = getWLCGStatus();
	if ((code == TStatusCode.SRM_SUCCESS) || (code == TStatusCode.SRM_PARTIAL_SUCCESS)) {
	    if (!isFinalStatusReturned()) {
		TSRMLog.info(this.getClass(), null, "event=cleanUpDelayedWaittingForStatusCall", null);
		return;
	    }
	    // check whether all requests are done;
	    if (this.isSubjectToCleanup()) {
		cleanMe();
	    } else {
		TSRMLog.info(this.getClass(), null, "event=cleanUpDelayed", "rid="+getID());
	    }
	    return;
	}
    ...
    private boolean isFinalStatusReturned() {
	...
	while (iter.hasNext()) {
	    ...
	    if (r.getLastQueriedStatusCode() != r.getReturnStatus().getStatusCode()) {
		TSRMLog.info(this.getClass(), null, "event=finalStatusNotQueriedByClient", "record="+r.getLastQueriedStatusCode());
		return false;
	    }
	}
	return true;
	...
	r.setLastQueriedStatus(r.getReturnStatus().getStatusCode());
