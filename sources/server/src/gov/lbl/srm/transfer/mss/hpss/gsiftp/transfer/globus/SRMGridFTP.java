...
package gov.lbl.srm.transfer.mss.hpss.gsiftp.transfer.globus;
...
import gov.lbl.srm.transfer.mss.hpss.gsiftp.transfer.ISRMFileTransfer;
...
import gov.lbl.srm.transfer.mss.hpss.gsiftp.util;
...
public void setLogger(Log logger, boolean silent,boolean debug) {
  this.logger = logger;
  this.silent = silent; 
  this.debug = debug;
}
...
public SRMGridFTP(String fromu, String tou) {
	try {
        ...
        util.printEventLog(_theLogger,"SRMGridFTP",inputVec,silent);
		status = e.toString();
	}
}
...
public void transfer(String fromu, String tou) {
	try {
        ...
        inputVec = new Vector ();
        inputVec.addElement(e.toString());
        util.printEventLog(_theLogger,"SRMGridFTP.transfer",inputVec,silent);
		status = e.toString();
	}
}
...
public void cancel() {
    ...
    util.printEventLog(_theLogger,"GridFTP-Cancelled",inputVec,silent); 
}
...
public void setSourceUrl(String surl) throws IOException {
    ...
    try {
        ...
        inputVec = new Vector ();
        inputVec.addElement(e.toString());
        util.printEventLog(_theLogger,"SRMGridFTP.setSourceUrl",inputVec,silent);
		status = e.toString();
    }
}
...
public void setDestinationUrl(String surl) throws IOException {
    ...
    try {
        ...
        inputVec = new Vector ();
        inputVec.addElement(e.toString());
        util.printEventLog(_theLogger,"SRMGridFTP.setDestinationUrl",inputVec,silent);
		status = e.toString();
    }
}
...
public void transferSync(String fromu, String tou) {
	try {
        ...
        inputVec = new Vector ();
        inputVec.addElement(e.toString());
        util.printEventLog(_theLogger,"SRMGridFTP.transferSync",inputVec,silent);
        if(!transferok) {
		  e.printStackTrace();
		  status = e.toString();
        }
    ...
    try {
        ...
        if (tmode == SRMTransferMode.GET) {
            ...
            util.printEventLog(_theLogger,"TransferSync",inputVec,silent);
            get();
        ...
        util.printEventLog(_theLogger,"Exception",inputVec,silent);
        if(!transferok) {
          e.printStackTrace();
		  status = e.toString();
        }
...
public void thirdparty() throws IOException {
	try {
		...
		toclient.setProtectionBufferSize(16384);
	...
	try {
        ...
        */
        ...
        if (client.isFeatureSupported("DCAU")) {
          ...
          util.printEventLog(_theLogger,"DCAU",inputVec,silent);
          if(debug) {
...
// added 5/5/08 AS
	    ...
	    if (transferType != Session.TYPE_ASCII) {
	        remoteSize = client.getSize(path);
	    }
	    client.close();
    } catch (org.globus.ftp.exception.ServerException e) {
        inputVec = new Vector ();
