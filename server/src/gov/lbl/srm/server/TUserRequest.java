...
public abstract class TUserRequest implements Comparable {
    ...
    protected TSRMMutex _requestCollectionMutex = new TSRMMutex();
    ...
    public boolean isSubjectToCleanup() {
	if (TSRMUtil.acquireSync(_requestCollectionMutex)) {
	    HashSet reqSet = new HashSet(_requestCollection.values());
	    //result.setTotalNumFilesInRequest(new Integer(reqSet.size()));
	    ...
	    while (iter.hasNext()) {
		TSRMRequest r = (TSRMRequest)(iter.next());
		if (!r.isSubjectToRelease()) {
		    TSRMLog.debug(this.getClass(), null, "event=foundNotcleanable", r.getIdentifier()+r.getReturnStatus().getStatusCode());
		    return false;
		}
	    }
	    TSRMUtil.releaseSync(_requestCollectionMutex);				  
    ...
    public void cleanUp() {};
    public void cleanMe(TStatusCode code) { // input is user req code
	TSRMLog.info(this.getClass(), null, "event=cleanWithStatus", "code="+code);
	...
	TSRMLog.info(this.getClass(), null, "event=clean", "rid="+getID());
