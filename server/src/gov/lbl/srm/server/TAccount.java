...
public class TAccount {
    ...
    private TSRMMutex   _spaceTokenAccessMutex = new TSRMMutex();
    
    private TSRMMutex _ridGenerateMutex = new TSRMMutex();
	...
	*/
    ...
    public void cleanUp() {
	// donnt lock mutex, remove user request needs it
	TSRMLog.info(this.getClass(), null, "event=cleanUp id="+getID(), "size="+_requestCollection.size());
    ...
    public void removeRequest(TUserRequest r) {
	if (TSRMUtil.acquireSync(_requestMutex)) {
	    _requestCollection.remove(r.getID());
	    TSRMUtil.releaseSync(_requestMutex);
	}
    }
