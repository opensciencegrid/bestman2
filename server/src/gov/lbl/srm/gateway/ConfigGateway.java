...
public class ConfigGateway extends Config{
    ...
    public static final String _DefConfigGatewayTokenCompName ="xrootdTokenCompName";
  ...
  //public static StaticToken[] _staticTokenList = null;
	...
	if (prop.getProperty(ConfigGateway._DefConfigGatewayTokenCompName) != null) {
	    _xrootdTokenCompName = prop.getProperty(ConfigGateway._DefConfigGatewayTokenCompName);
	    prop.put(ConfigGateway._DefConfigGatewayTokenCompName, _xrootdTokenCompName);
	} else {
	    //TSRMUtil.startUpInfo(".........assuming default xrootd token component name:"+_xrootdTokenCompName);
	}

	String enableRecursionInLs = prop.getProperty(ConfigGateway._DefConfigEnableRecursionInLs);
	...
	String checkXrootdFS = prop.getProperty(ConfigGateway._DefConfigCheckXrootdFS);
	if ((checkXrootdFS != null) && (checkXrootdFS.equalsIgnoreCase("false"))) {
	    _doCheckSizeWithXrootdFS = false;
	}
	
	String checkGsiftp = prop.getProperty(ConfigGateway._DefConfigCheckSizeGsiftp);
	...
	*/
	...
	String checkXrootdToken = prop.getProperty(ConfigGateway._DefConfigCheckXrootdToken);
	...
	String fsc = prop.getProperty(ConfigGateway._DefConfigGatewayFSAC);	
	if (fsc != null) {
	    try {
		_fsConcurrency = Integer.parseInt(fsc);
	    } catch (Exception e) {
		e.printStackTrace();
		TSRMUtil.startUpInfo("Error: failed to read integers from: "+ConfigGateway._DefConfigGatewayFSAC+" value="+fsc);
		System.exit(1);
	    }
	}
	return true;
